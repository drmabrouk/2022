    ...
    const SM_UI = {
        ...
        handleAjaxError: function(err, customMsg = 'حدث خطأ أثناء تنفيذ العملية') {
            ...
            if (err instanceof Response) {
                msg = `Network Error: ${err.status} ${err.statusText}`;
                err.text().then(body => {
                    console.error('Error Body:', body);
                    if (body.includes('0') && body.length < 5) {
                        this.showNotification(customMsg + ': WordPress returned 0. (Missing Action/Perms)', true);
                    } else {
