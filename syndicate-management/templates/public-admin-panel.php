            ...
            <div>
                ...
                <div style="display: inline-flex; align-items: center; padding: 6px 16px; background: #f0f4f8; color: #111F35; border-radius: 12px; font-size: 11px; font-weight: 700; margin-top: 8px; border: 1px solid #cbd5e0; line-height: 1.4; gap: 8px;">
                    <div style="color: #4a5568;">
                        ...
                        elseif (in_array('sm_general_officer', $roles)) echo 'مسؤول النقابة العامة';
                        elseif ($is_syndicate_admin) echo 'مسؤول نقابة فرعية';
    ...
    <div class="sm-admin-layout" style="display: flex; min-height: 800px;">
        ...
        <div class="sm-sidebar" style="width: 280px; flex-shrink: 0; background: <?php echo $appearance['sidebar_bg_color']; ?>; border-left: 1px solid var(--sm-border-color); padding: 15px 0;">
            <ul style="list-style: none; padding: 0; margin: 0;">
                ...
                <?php if (SM_Access::is_module_enabled('members')): ?>
                ...
                <?php endif; ?>
                <?php if (SM_Access::is_module_enabled('finance')): ?>
                ...
                <?php endif; ?>
                <?php if (SM_Access::is_module_enabled('licenses')): ?>
                ...
                <?php endif; ?>
                <?php if (SM_Access::is_module_enabled('services')): ?>
                ...
                <?php endif; ?>
                <?php if (SM_Access::is_module_enabled('surveys')): ?>
                ...
                <?php endif; ?>
                <?php if (SM_Access::is_module_enabled('branches')): ?>
                ...
                case 'advanced-settings':
                    if ($is_admin || $is_sys_admin): $sub = $_GET['sub'] ?? 'staff'; ?>
                        <div class="sm-tabs-wrapper" style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #eee; overflow-x: auto; white-space: nowrap; padding-bottom: 10px;">
                            ...
                            <button class="sm-tab-btn <?php echo ($sub == 'permissions') ? 'sm-active' : ''; ?>" onclick="smOpenInternalTab('role-permissions-tab', this)">صلاحيات الوصول</button>
                        </div>
                        ...
                        <div id="role-permissions-tab" class="sm-internal-tab" style="display: <?php echo ($sub == 'permissions') ? 'block' : 'none'; ?>;">
                            <div style="background:#fff; border:1px solid #e2e8f0; border-radius:12px; padding: 25px;">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;"><h3 style="margin:0;">التحكم في صلاحيات الوصول للمودولات</h3><button onclick="smSaveRolePermissions('<?php echo wp_create_nonce('sm_admin_action'); ?>')" class="sm-btn" style="width:auto; padding:0 30px;">حفظ الصلاحيات</button></div>
                                <div class="sm-table-container"><table class="sm-table"><thead><tr><th>المودول / الخدمة</th><th>مدير النظام</th><th>مسؤول عام</th><th>مسؤول فرع</th><th>عضو النقابة</th></tr></thead><tbody>
                                <?php
                                $perms = get_option('sm_role_permissions', SM_Access::get_default_permissions());
                                $modules = [
                                    'members' => 'إدارة الأعضاء والطلبات',
                                    'finance' => 'المحاسبة والمالية',
                                    'licenses' => 'التراخيص والتصاريح',
                                    'services' => 'الخدمات الرقمية',
                                    'surveys' => 'الاختبارات المهنية',
                                    'branches' => 'إدارة الفروع',
                                    'system' => 'إعدادات النظام والنسخ'
                                ];
                                $roles_list = ['sm_system_admin', 'sm_general_officer', 'sm_syndicate_admin', 'sm_syndicate_member'];
                                foreach ($modules as $m_key => $m_label): ?>
                                    <tr><td><strong><?php echo $m_label; ?></strong></td>
                                    <?php foreach ($roles_list as $r_key): ?>
                                        <td style="text-align:center;"><input type="checkbox" class="sm-perm-check" data-role="<?php echo $r_key; ?>" data-mod="<?php echo $m_key; ?>" <?php checked($perms[$r_key][$m_key] ?? false); ?>></td>
                                    <?php endforeach; ?></tr>
